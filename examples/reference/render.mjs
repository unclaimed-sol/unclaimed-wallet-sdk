...
export function render(state, mode) {
  ...
  const locked = state.busy || state.error?.retryAction === "same_key";
  const form = (action, label) =>
    `<form method="post" action="/"><input type="hidden" name="csrf" value="${e(state.csrf)}"><input type="hidden" name="action" value="${action}"><button ${state.busy ? "disabled" : ""}>${e(label)}</button></form>`;
  return `<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Unclaimed API analysis preview</title><link rel="stylesheet" href="/style.css"><script src="/loading.js" defer></script><main><p class="eyebrow">UNCLAIMED API · ANALYSIS PREVIEW</p><h1>Explore wallet opportunities</h1><p class="notice"><strong>Execution unavailable.</strong> This integration cannot build, sign, submit or record transactions.</p><p>${mode === "fixture" ? "Offline synthetic fixture. No provider calls, real wallet results or charges." : "Local reference integration using the configured platform. Each requested page is metered; no payment is collected here."}</p>
  <form method="post" action="/"><input type="hidden" name="csrf" value="${e(state.csrf)}"><input type="hidden" name="action" value="start"><label>Wallet address <input name="wallet" required maxlength="44" value="${e(state.wallet ?? "")}" ${mode === "fixture" || locked ? "readonly" : ""}></label><label>Accounts per page <input type="number" name="limit" min="1" max="200" value="${e(state.limit ?? 20)}" ${mode === "fixture" || locked ? "readonly" : ""} required></label><button ${locked ? "disabled" : ""}>Start new analysis</button></form><p role="status" aria-live="polite">${state.busy ? "Loading analysis. Keep this page open; no automatic retries." : ""}</p>
  ${state.error ? `<p role="alert">${e(state.error.message)} Retry guidance: ${e(state.error.retryAction)}.${["same_key", "new_key"].includes(state.error.retryAction) && state.retryAt > Date.now() ? ` Wait ${Math.ceil((state.retryAt - Date.now()) / 1000)} seconds before retrying.` : ""}</p>${["same_key", "new_key"].includes(state.error.retryAction) ? form("retry", state.error.retryAction === "same_key" ? "Retry same attempt" : "Start a new attempt for this page") : state.error.retryAction === "restart_snapshot" ? form("restart", "Restart expired snapshot with the same settings") : ""}` : ""}
